...
void FileManager::readFromFile(string filename, vector<Tile>& tiles)
{
	...
	while(!file.eof())
	{
		Vector2f 	position;
		string		type;
		file>>type>>position.y>>position.x;
		std::cout << position.x << ":" << position.y << "   " << type <<std::endl;
		Sprite 	sprite;
				sprite.setTexture(MediaBucket.getTexture(type));
		Tile 	temp;
				temp.setPosition(Vector2f(position.x, position.y));
...
void FileManager::writeToFile(string filename, vector<Tile>& tiles)
{
	...
	for(int i=0; i<tiles.size(); i++) {
		...
		file<<left<<setw(10)<<setfill(' ')<<type<<setw(10)<<position.y<<setw(10)<<position.x<<endl;
	}
}
